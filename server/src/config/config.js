...
for (const file of envFiles) {
    if (fs.existsSync(file)) {
        ...
        loadedEnvFile = file;
        console.log(
            `[Config] Loaded environment variables from: ${path.basename(file)}`
        );
...
if (!loadedEnvFile) {
    console.warn(
        `[Config] Warning: No .env file found. Using default values and system environment variables.`
    );
}
...
const config = {
    ...
    jwt: {
        ...
        refreshExpirationDays:
            parseInt(process.env.JWT_REFRESH_EXPIRATION_DAYS, 10) || 30
    },
